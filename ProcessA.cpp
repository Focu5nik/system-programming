...
int main(void)
{
	LPCTSTR pipename = (LPCTSTR)"\\\\.\\pipe\\mynamedpipe";					// имя канала
	...
	hProcessB = OpenProcess(
		...
		GetProcessID((char*)"ProcessB.exe"));	                            // ID процесса (в данном случае - процесса B)
...
DWORD GetProcessID(char* process) {
	...
	if (Snapshot != INVALID_HANDLE_VALUE) {
		...
		do {
			if (strcmp((char*)processEntry.szExeFile, process) == 0) {
